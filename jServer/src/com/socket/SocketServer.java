...
public class SocketServer implements Runnable {
    ...
    public void SendAllUsers(int ID, Message msg)
    {
    	for(int i=0; i<clientCount; i++)
    	{
    		if(!al.contains(clients[i].username))
    		{
    			al.add(clients[i].username);
    		}
    	}
    	   String users[]= sendusers();
           for(int i=0; i<users.length; i++)
            {
            	if(!(al.contains(users[i])))
            		
            	{
            		clients[findClient(ID)].send(new Message("userlist", "SERVER", users[i], msg.sender));
            	}
            }
    ...
    int pos = findClient(ID);
        if (pos >= 0){  
            ...
            al.remove(clients[pos].username);
